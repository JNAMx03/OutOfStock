...
// Almacenamiento temporal en memoria para mock data
let mockStoresCache: Store[] = [];
...
export async function getAllStores(userId: string): Promise<Store[]>{
    try{
        ...
        // Por ahora, retornamos datos mock con caché en memoria
        ...
        // Si el caché está vacío, inicializarlo con tienda de ejemplo
        if (mockStoresCache.length === 0) {
            mockStoresCache = [
                {
                    id: 'store-initial-1',
        ...
        // Retornar una copia del caché para evitar mutaciones
        return JSON.parse(JSON.stringify(mockStoresCache));
    }catch(error){
...
export async function updateStore( storeId: string,  data: UpdateStoreData): Promise<Store> {
    try {
        ...
        // Buscar la tienda en el caché
        const storeIndex = mockStoresCache.findIndex(s => s.id === storeId);
        ...
        const originalStore = mockStoresCache[storeIndex];
        
        // Crear objeto actualizado preservando todos los datos
        const updatedStore: Store = {
            ...originalStore,
            name: data.name ?? originalStore.name,
            description: data.description ?? originalStore.description,
            type: data.type ?? originalStore.type,
            status: data.status ?? originalStore.status,
            // Merge address correctamente
            address: {
                ...originalStore.address,
                ...(data.address || {}),
            },
            phone: data.phone ?? originalStore.phone,
            email: data.email ?? originalStore.email,
            color: data.color ?? originalStore.color,
            // Merge inventorySettings correctamente
            inventorySettings: {
                ...originalStore.inventorySettings,
                ...(data.inventorySettings || {}),
            },
            id: storeId, // Garantizar que el ID no cambia
            updatedAt: new Date().toISOString(),
        };
        
        // Actualizar en el caché
        mockStoresCache[storeIndex] = updatedStore;
...
export async function deleteStore(storeId: string): Promise<void>{
    try{
        ...
        // Cambiar el estado a 'closed' (soft delete)
        mockStoresCache[storeIndex].status = 'closed';
        mockStoresCache[storeIndex].updatedAt = new Date().toISOString();
...
/** 
*********************INFORMACION  DE ANTES DEL CAMBIO, COMO DIJO CLAUDE***************************
*/
...
// export async function removeUserFromStore(  storeId: string,  userId: string): Promise<void> {
//   try {
//     console.log('🚫 Removiendo usuario de tienda:', storeId);
